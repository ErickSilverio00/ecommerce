...
export default function Roupas() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const produtosRoupas = produtos.filter(
          (produto) => produto.categoria_produto === "Roupas"
        );
        setProdutosDisponiveis(produtosRoupas);
      } catch (error) {
        console.error(error);
      }
    };
  ...
  return (
    <div>
      ...
      <ContainerPrincipal>
        <ContainerSlides>
          <ContainerImagem src={fundoRoupas} />
        </ContainerSlides>
        <ContainerBlocoProdutos id="containerBlocoProdutos">
          <ContainerPrimeiraLinha>
            <TituloSubtituloSecoes title="Roupas" />
            <ContainerFiltroOrdenacao>
              <CampoSelecionar
                label="Ordenar por"
                variante="outlined"
