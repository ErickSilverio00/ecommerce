...
export default function Acessorios() {
  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        const produtosAcessorios = produtos.filter(
          (produto) => produto.categoria_produto === "Acessórios"
        );
        setProdutosDisponiveis(produtosAcessorios);
      } catch (error) {
        console.error(error);
      }
    };
  ...
  return (
    <div>
      ...
      <ContainerPrincipal>
        <ContainerSlides>
          <ContainerImagem src={fundoAcessorios} />
        </ContainerSlides>
        <ContainerBlocoProdutos id="containerBlocoProdutos">
          <ContainerPrimeiraLinha>
            <TituloSubtituloSecoes title="Acessórios" />
            <ContainerFiltroOrdenacao>
              <CampoSelecionar
                label="Ordenar por"
                variante="outlined"
